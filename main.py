...
class Logger:
    def __init__(self, args):
        ...
        self.logger = open(os.path.join(args.checkpoint_dir, f'{exp_name}-val-logger.log'), 'a')
...
def make_args_parser():
    ...
    parser.add_argument(
        "--pretrained_load_scope", default="detector", type=str, choices=["detector", "full"],
        help="load scope for --pretrained_weights: detector-only for Vote2Cap init, full for ll3da generalist ckpt."
    )
    ...
    parser.add_argument(
        "--enable_spatial_attention", default=False, action='store_true',
        help='whether to enable SceneVerse spatial attention in captioner.'
    )
    parser.add_argument(
...
def main(local_rank, args):
    ...
    if args.test_only:
        ...
        try:
            ...
            # Strictly load full model weights for evaluation.
            # Do not silently fallback to detector-only loading.
            model.load_state_dict(checkpoint['model'], strict=False)
            print('Loaded full checkpoint for evaluation.')
        except Exception as e:
            raise RuntimeError(
                f'Failed to load full checkpoint for evaluation: {args.test_ckpt}\n{e}'
            )
    ...
    # training phase
        ...
        if args.pretrained_weights is not None:
            ...
            if args.pretrained_load_scope == "full":
                loaded_dict = checkpoint["model"]
            else:
                loaded_dict = {k: v for k, v in checkpoint["model"].items() if k.startswith("detector.")}
